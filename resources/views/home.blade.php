...
<head>
    ...
    <title>PXL.Widgets</title>
    <link rel="stylesheet" href="/css/app.css">
</head>
<body class="bg-gray-100 h-full flex items-center justify-center">
    <main class="p-8 rounded border border-gray-900 space-y-2">
        <form class="flex flex-col space-y-2 text-sm p-4 border" action="/link" method="post">
            @csrf
            <input type="text" name="link" class="px-3 py-1 rounded w-96" placeholder="Copy the JSON link" value="{{env("APP_URL") . "/challenge.json"}}">
            <input type="submit" value="Process" class="bg-green-400 rounded px-2 py-0.5 w-max text-xs">
            @error('link')
        ...
        <p class="text-center">OR upload/drag the JSON</p>
        <form class="flex items-center justify-center p-4 rounded border relative" action="/file" method="post" enctype="multipart/form-data">
            @csrf
            <svg class="h-12" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
            ...
            <input type="file" id="file" name="file" class="absolute inset-0 z-50 m-0 p-0 w-full h-full outline-none opacity-0" accept=".json" onchange="form.submit()">
        </form>
        @error('file')
        <ul class="text-xs text-red-700">
        <li>*{{$message}}</li>
        ...
        @enderror
        @if (session('message'))
            <p class="text-sm text-green-700 text-center">{{session('message')}}</p>
        @endif
        <div class="pt-8">
            <a href="/export" class="rounded shadow bg-blue-700 text-white text-sm px-4 py-2">Export Database to CSV</a>
            <a href="{{ route('purge') }}"
                class="text-sm px-4 py-2 bg-red-700 text-white shadow rounded mt-4"
                onclick="event.preventDefault();
                    document.getElementById('purge-form').submit();">Purge Database</a>
